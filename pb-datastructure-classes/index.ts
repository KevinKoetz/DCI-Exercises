...
class TV {
  private constructor(brand: string, instance?: TV | undefined) {
    this.brand = brand;
  }
  static defaults = { channel: 1, volume: 50 };
  brand;
  channel = TV.defaults.channel;
  volume = TV.defaults.volume;
  private static instance: undefined | TV;

  static getInstance(brand: string) {
    if (TV.instance) return TV.instance;
    else {
  ...
  static deleteInstance() {
    if (TV.instance) TV.instance = undefined;
  }
...
type TestConstructor = {
  new (name: string): Test
}
type Test = { name: string; getName: () => string}
