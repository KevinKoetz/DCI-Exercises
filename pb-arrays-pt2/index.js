...
const findGreater = (numbers, limit) => numbers.filter(number => number > limit);
...
const longestWord = (str) => str.split(" ").reduce((word1, word2) => word1.length >= word2.length ? word1 : word2);
...
const findVowels = (str) => { var _a, _b; return (_b = (_a = str.match(/[aeiou]/g)) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0; };
...
const missingNums = (nums) => (nums.sort((a, b) => a - b).find((num, index) => num !== index + 1) - 1) || 10;
...
sumOfCubes([2]) ➞ 8
sumOfCubes([]) ➞ 0 */
const sumOfCubes = (nums) => nums.reduce((prev, cur) => prev + Math.pow(cur, 3), 0);
...
const dictionary = (start, words) => words.filter(word => word.startsWith(start));
...
Do not include 0.
*/
const evenNums = (max) => Array(Math.floor(max / 2)).fill(0).map((val, index) => (index + 1) * 2);
