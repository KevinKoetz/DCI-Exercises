...
books.sort((bookA, bookB) => {
    ...
    if(authorSurnameA < authorSurnameB) return -1
    if(authorSurnameA > authorSurnameB) return 1
    return 0
})

const bookList = document.querySelector<HTMLUListElement>(".book-list")
const template = document.querySelector<HTMLTemplateElement>("#bookTemplate")

const bookFragments: DocumentFragment[] = books.map(bookData => {
    const bookFragment = template.content.cloneNode(true)
    if(bookFragment instanceof DocumentFragment) {

        const img = bookFragment.querySelector<HTMLImageElement>("img")
        const title = bookFragment.querySelector<HTMLElement>("h3")
        const author = bookFragment.querySelector<HTMLElement>("p")
        const badge = bookFragment.querySelector<HTMLElement>(".badge")
        const section = bookFragment.querySelector<HTMLElement>("section")
