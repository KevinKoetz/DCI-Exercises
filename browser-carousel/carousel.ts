...
export class Carousel extends HTMLElement implements WebComponent {
  constructor() {
    super();
    ...
    if (!(current instanceof HTMLImageElement))
      throw new Error(
        "Element with id current is required to be an HTMLImageElement"
      );
    ...
    if (!(prevImg instanceof HTMLImageElement))
      throw new Error(
        "Element with id prev is required to be an HTMLImageElement"
      );
    ...
    if (!(nextImg instanceof HTMLImageElement))
      throw new Error(
        "Element with id next is required to be an HTMLImageElement"
      );
    ...
    if (!(slider instanceof HTMLDivElement))
      throw new Error(
        "Element with id slider is required to be an HTMLDivElement"
      );
    this.slider = slider;

    this.imageUrlArray = []
    const imageUrls = this.getAttribute("imageUrls");
    if (imageUrls) this.attributeChangedCallback("imageUrls", "", imageUrls);
    ...
    shadow
      ...
      ?.addEventListener("click", this.handlePrevious.bind(this));
  }
  ...
  handleNext() {
    if(this.slider.classList.contains("moveNext") || this.slider.classList.contains("movePrev")) return;
    ...
    switch (name) {
      case "imageUrls":
        ...
        if (!Array.isArray(imageUrls))
          throw new Error(
            "Invalid String passed in imageUrls Attribute. You need to pass a JSON string[]"
          );
...
export function createImageCarousel(images: string[]) {
